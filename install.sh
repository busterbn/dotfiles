...
# is applied, so restore.sh can always put the machine back. Runs only once —
# remove the guard by running restore.sh.
set -e

DIR="$HOME/dotfiles"

if [ -f "$DIR/.bootstrapped" ]; then
    echo "Already bootstrapped ($DIR/.bootstrapped exists). Run $DIR/restore.sh to undo first."
    exit 1
fi
...
# --- Backup of everything the make targets can touch ---
BACKUP="$DIR/.backup"
rm -rf "$BACKUP"
